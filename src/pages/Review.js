  ...
  useEffect(() => {
    const GetAllPost = () => {
      if (document.cookie.split(';').some((item) => item.trim().startsWith('accessToken='))) {
        ...
        return fetch(`${apiUrl}/posts-review?page=${params.page}&page_size=${NumberPost}&sort=${sort[SortNumber]}&sortty=${sortty[SortNumber]}`, requestOptions)
          .then(res => res.json())
          .then(data => {
